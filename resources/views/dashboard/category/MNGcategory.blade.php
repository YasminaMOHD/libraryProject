...
<head>
    ...
    <link rel="stylesheet" href="{{asset('css/formStyle.css')}}">    <link rel="stylesheet" href="{{asset('css/MNG.css')}}">
        ...
        <div class="row">
            ...
            <div class="col-12">
                <div class="container-content">
                    <p>مكتبة اقرأ <span>/ ادارة التصنيفات</span></p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action" style="display: flex">
                                    <form method="get" action="{{url('category/show/'.$cat->id)}}">
                                        @csrf
                                        <button class="btn active-btn btn-primary mb-2"><i class="fas fa-edit"></i></button>
                                    </form>
                                    <form method="get" action="{{url('category/delete/'.$cat->id)}}" class="mr-2">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('هل متأكد من أنك تريد حذف {{$cat->name}}'   +' ؟ ')" value="{{$cat->name}}" class="delete-btn btn active-btn btn-danger"><i class="fas fa-trash-alt"></i></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
